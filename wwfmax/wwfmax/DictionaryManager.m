    ...
    if(item->index == 0) {
        do {
            ...
            if(offset != 0) {
                ...
                assert(item->childLetterFormatOffset < NUMBER_OF_ENGLISH_LETTERS);
                ++item->index;
                goto LOOP_END;
            }
            item--;
...
LOOP_END:;
    ...
    do {
        ...
        item->childLetterFormatOffset = (char)ffs(item->childListFormat);
        item->childListFormat >>= item->childLetterFormatOffset--;
        assert(item->childLetterFormatOffset < NUMBER_OF_ENGLISH_LETTERS);
    } while(!(node & EOW_FLAG));
    
    assert(mgr->stackDepth > 1 && mgr->stackDepth <= BOARD_LENGTH);
    strncpy(outWord, mgr->tmpWord, 15); //the compiler can unroll this with a constant length, so it's faster than a dynamic strncpy
