...
static unsigned int TheLookupTable[TWO_UP_EIGHT];
...
bool TestForValidWord(const unsigned char *TheWord){
	int Length = (int)strlen((const char*)TheWord);
	int X;
	int Y;
	for ( X = 0; X < Length; X++ ) {
...
int StringToPositiveInt(char* TheNumberNotYet){
	int Result = 0;
	int X;
	int Length = (int)strlen(TheNumberNotYet);
...
TnodePtr TnodeInit(unsigned char ChapIndex, TnodePtr OverOne, char WordEnding, char Leveler, int StarterDepth, TnodePtr Parent, char IsaChild, char StartListPosition){
	...
	Result->MaxChildDepth = (char)StarterDepth;
	Result->Next = OverOne;
	Result->Child = NULL;
...
void TnodeSetMaxChildDepth(TnodePtr ThisTnode, int NewDepth){
	ThisTnode->MaxChildDepth = (char)NewDepth;
}
...
void TnodeCalculateInternalValues(TnodePtr ThisTnode){
	...
	TheBytes[3] = (char)((ThisTnode->MaxChildDepth) << 1) + ThisTnode->EndOfWordFlag;
}
...
void BreadthQueuePopulateReductionArray(BreadthQueuePtr ThisBreadthQueue, TnodePtr Root, TnodePtr ****Holder){
	int InsertionPosition[MAX_LEN][SIZE_OF_CHARACTER_SET][2];
	int CMCD;
	char CLetterIndex;
	char CDCstatus;
	memset(InsertionPosition, 0, 2*MAX_LEN*SIZE_OF_CHARACTER_SET*sizeof(int));
...
ArrayDawgPtr ArrayDawgInit(unsigned char **Dictionary, int *SegmentLenghts, const char *outFile){
	int X;
	int Y;
	int Z;
	int W;
	...
	for ( X = MIN_LEN; X <= MAX_LEN ; X++ ) Result->NumberOfStrings += SegmentLenghts[X];
	...
	int ***NodeGroupCounter= (int ***)malloc(MAX_LEN*sizeof(int **));
	int ***NodeGroupCounterInit = (int ***)malloc(MAX_LEN*sizeof(int **));
	...
	TnodePtr ****HolderOfAllTnodePointers = (TnodePtr ****)malloc(MAX_LEN*sizeof(TnodePtr ***));
	for ( X = 0; X < MAX_LEN; X++ ) {
	...
	FILE *Text;
    char *outFileText = calloc(strlen(outFile) + 5, sizeof(char));
    strcpy(outFileText, outFile);
    strcat(outFileText, ".txt");
	Text = fopen(outFileText,"w");
...
int createDataStructure(const WordInfo *info, const char *outFile) {
    ...
    unsigned char *AllWordsInEnglish[MAX_LEN + 1];
    for ( X = 0; X < (MAX_LEN + 1); X++ ) AllWordsInEnglish[X] = NULL;
	...
	int FirstLineIsSize = info->numWords;
	
	printf("\n  Number-Of-Words = |%d|\n", FirstLineIsSize);
	int DictionarySizeIndex[MAX_LEN + 1];
	for ( X = 0; X <= MAX_LEN; X++ ) DictionarySizeIndex[X] = 0;
	unsigned char **LexiconInRam = (unsigned char**)malloc(FirstLineIsSize*sizeof(unsigned char *));
	
	// The first line is the Number-Of-Words, so read them all into RAM, temporarily.
    int itr = 0;
	for ( X = 0; X < FirstLineIsSize; X++ ) {
        char *word = &info->words[X * BOARD_LENGTH];
		int LineLength = info->lengths[X];
        if(LineLength == 0) {
            continue;
        }
		if ( LineLength <= MAX_LEN ) DictionarySizeIndex[LineLength] += 1;
		else {
			printf("The word in position |%d| is too long.  EXIT.\n", X);
			return 0;
		}
		LexiconInRam[itr] = (unsigned char *)calloc((LineLength + 1), sizeof(unsigned char));
		strncpy((char*)LexiconInRam[itr], word, LineLength);
        assert(word);
        assert(LineLength > 1);
        assert(strncmp((char*)LexiconInRam[itr], word, LineLength) == 0);
        itr++;
	}
    FirstLineIsSize = itr;
    int CurrentLength;
	...
	for ( X = 2; X < (MAX_LEN + 1); X++ ) AllWordsInEnglish[X] = (unsigned char *)malloc(X*DictionarySizeIndex[X]*sizeof(unsigned char));
	
	int CurrentTracker[MAX_LEN + 1] = {0};
	memset(CurrentTracker, 0, (MAX_LEN + 1)*sizeof(int));
	unsigned char CurrentWordByIndex[MAX_LEN];
	// Copy all of the words into the "LetterIndex" format "AllWordsInEnglish" array.
	for ( X = 0; X < FirstLineIsSize; X++ ) {
		CurrentLength = (int)strlen((char*)LexiconInRam[X]);
        //assert(CurrentLength == info->lengths[X]);
	...
	ArrayDawgInit(AllWordsInEnglish, DictionarySizeIndex, outFile);
